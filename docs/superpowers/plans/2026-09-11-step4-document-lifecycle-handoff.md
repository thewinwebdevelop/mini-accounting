...
> **Owner updates after handoff:** Read [`../specs/2026-09-11-step4-owner-decisions.md`](../specs/2026-09-11-step4-owner-decisions.md) before using this plan. O1–O7 override conflicting text below, including cancellation/stock effects, workflow-level manual Sheets/Drive sync, returning to draft, and hybrid workflow completion. All document kinds now require explicit `complete` before unlocking the next step. Unresolved behavior in the addendum must be clarified with the owner before implementing dependent tasks. This handoff is retained as the original reference and is not yet a reconciled implementation plan.
